...
public class MainWrapper {
  ...
  public static void ex02() {
    String fullName = "apple.jpg";
    int dot = fullName.lastIndexOf(".");  // 파일명에는 . 이 포함될 수 있기 때문에 마지막 . 을 기준으로했다. (파일명만 있거나 확장자 안에도 .이 들어가는 경우가 있지만 그런 경우는 예외로 했다.)
  ...
  public static void ex03() {
    ...
    sc.close();
  } // 타임스탬프값을 임시파일 이름으로 사용하기도 한다.
  /*
   * ex03, Tip)
   * 
   * System.in은 InputStream이기 때문에
   * 
   * BufferedReader reader = new BufferedReader(new InputStreamReader(System.in));  
   * System.out.print("변환 전 파일명 >>> ");
   * beforeName = reader.readLine();
   *
   * 으로도 스캐너 없이 문자입력을 받을 수 있다.
   */
