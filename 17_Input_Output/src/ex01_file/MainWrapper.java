...
public class MainWrapper {
  ...
  public static void ex02() {
    ...
    try {
      ...
      if(file.exists()) {
        ...
        file.createNewFile(); // 반드시 예외 처리를 해야하는 코드(Checked Exception인 IOException 발생)    // 내용 없는 파일을 만들 일은 잘 없다.
  ...
  public static void ex03() {
    ...
    for(int i = 0; i < files.length; i++) {
      ...
      // File 객체 크기
      long size = file.isFile() ? file.length() : 0;  // 파일은 바이트 단위로 크기를 저장, 디렉터리는 크기가 없으므로 0으로 저장
...
// 1024 byte --> 1 KB
// 강사님 경험상 Java 1.8 버전 이상 Windows 플랫폼에서도 슬래시(/)가 인식된다. (이전 버전은 확인 안 해보심)